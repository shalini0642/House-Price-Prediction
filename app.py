...
# --------------------------------------------------
required_files = ["model.pkl", "metrics.pkl", "predictions.pkl"]
...
if not all(os.path.exists(file) for file in required_files):
    with st.spinner("Training model for the first time..."):
        ...
        joblib.dump(model_train, "model.pkl")
        joblib.dump({
...
MedInc = st.sidebar.slider("Median Income", 0.0, 15.0, 3.0, 0.1,
    help="Median income in the block.")
HouseAge = st.sidebar.slider("House Age", 1.0, 60.0, 20.0, 1.0)
...
ax.scatter(predictions["actual"], predictions["predicted"], alpha=0.5)
